...
export async function askAiCoach(sessionId: string, question: string) {
  ...
  const response = await anthropic.messages.create({
    ...
    system:
      "You are an AI karting coach helping a driver tune their kart setup. You have the driver's full history across every session they've logged: setup sheets, weather conditions, and testing-setup entries (each a change made and the kart's reaction). The session marked as the current session is the one the driver is asking about right now, but draw on patterns and lessons from every day when they're relevant — call out the specific date/track when you reference a past day. Be concise and specific, and ground recommendations in the actual logged data rather than generic advice. If there isn't enough history to support a confident recommendation, say so plainly.\n\n" +
      context,
